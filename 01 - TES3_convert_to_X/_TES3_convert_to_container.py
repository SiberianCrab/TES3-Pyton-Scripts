...
def validate_settings(s_name, max_length_name):
    if len(s_name) > max_length_name:
        print(f'\nERROR - defined "s_name" for records exceeds the maximum length of {max_length_name} characters.')
        return False
    return True
...
def generate_entry(full_id, full_path):
    ...
    {{
      ...
      "script": "{s_script}",
      "mesh": "{full_path}",
      "encumbrance": {s_encumbrance},
      "container_flags": "{s_container_flags}",
      "inventory": []
    }},''')
...
if __name__ == "__main__":
    ...
    print("\nTES3 Convert to Container Script\nby Siberian Crab\nv1.0.0")
    ...
    s_script = ""
    s_encumbrance = "100.0"
    s_container_flags = "IS_BASE"   # ORGANIC | RESPAWNS | IS_BASE
    ...
    output_file = "_TES3_convert_to_container.txt"
    log_file = "_TES3_convert_to_container_log.txt"
